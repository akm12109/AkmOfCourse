...
export default function AddCoursePage() {
  ...
  const resetForm = () => {
    setTitle(""); setInstructorName(""); setPrice(""); setOriginalPrice("");
    setDescription(""); setShortDescription(""); setImageUrl(""); setDataAiHint(""); setCategory("");
    setSkillLevel(""); setTags(""); setWhatYoullLearn(""); setRequirements("");
    setIsFeatured(false); setDuration(""); setLessonsCount(""); setSimplifiedCurriculum("");
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const courseData: Omit<Course, 'id' | 'rating' | 'reviewsCount' | 'studentsEnrolled' | 'createdAt' | 'instructorBio'> & { instructorName: string } = {
      title,
      instructorName,
    ...
    try {
      ...
      toast({ title: "Course Added!", description: `"${title}" has been successfully added.` });
      resetForm();
      ...
      <Card className="max-w-3xl mx-auto shadow-xl">
        <CardHeader>
          <CardTitle className="text-3xl flex items-center">
            <BookOpen className="mr-3 h-8 w-8 text-primary" /> Add New Course
            </CardTitle>
          <CardDescription>Fill in the details below to add a new course to the platform.</CardDescription>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="grid gap-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
             ...
             <div className="grid gap-2">
                ...
                <Input id="dataAiHint" value={dataAiHint} onChange={(e) => setDataAiHint(e.target.value)} placeholder="e.g. python code screen" />
            </div>
            ...
            <div className="grid gap-2">
                ...
                <Textarea id="simplifiedCurriculum" value={simplifiedCurriculum} onChange={(e) => setSimplifiedCurriculum(e.target.value)} placeholder="Module 1: Intro to Python...\nModule 2: Control Flow...\nOR JSON: [{id:'m1', title:'Mod1', lessons:[{...}]}]" rows={3} />
            </div>
